...
if [ "$SYSTEM" = 1 ]; then
    ...
    note "service   sudo ./setup.sh --system    installs it as a systemd unit instead"
fi
note "then point the game at this machine: /etc/hosts for an emulator"
note "(*.demonware.net -> its address), the DNS responder for a real PSP."
