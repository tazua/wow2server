...
def main() -> int:
    ...
    try:
        ...
        err = create_account(host, PORT, "Hiragamer98", NEW_PASSWORD)
        check(err == BD_AUTH_NO_ERROR,
              f"a create for 'Hiragamer98' is accepted (700), got {err}")
        mixed_t, mixed_pr = login(host, PORT, "hiragamer98")
        ...
        err = create_account(host, PORT, "HIRAGAMER98", IMPOSTOR_PASSWORD)
        check(err == BD_AUTH_CREATE_USERNAME_EXISTS,
              f"a create for the same name in another case is the same account: "
              f"refused (707), got {err}")
        owner_t, _pr = login(host, PORT, "hiragamer98")
