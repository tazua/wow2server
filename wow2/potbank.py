...
writes `max(10, round(0.9 * served))` -- the rating it read at sign-in, minus its
...
# What a brand-new account starts with on board 5. NOTHING in the game decides
# this -- the client reads its rating from us and the only constant it carries is
# the floor of 10, so a fresh account served nothing writes back max(10, 0) = 10
# and can never wager. 1000 is ours: comfortably clear of the floor, a round
# number under repeated 10% steps, and it makes a first win worth a visible +100.
RATING_START = 1000
...
def save(d: dict) -> bool:
    try:
        POT_DB.write_text(json.dumps(d, indent=1))
        return True
    except OSError:
        return False
...
def ensure_start(entity: int, name: str) -> int | None:
    """Give an account we have never rated the starting rating. Returns it, or None."""
    if not entity or statsdb.disabled():
        return None
    score, _rank, _n = statsdb.get(statsdb.RATING_BOARD, entity)
    if score:
        return None
    statsdb.put(statsdb.RATING_BOARD, entity, RATING_START, name)
    return RATING_START
