...
def run(keep: bool) -> int:
    ...
    print("the text")
    txt = bot.kit_text("Wormgamer98", "k7mfp2qx", "Wormhole", "<#555>")
    check("`k7mfp2qx`" in txt and "**Wormgamer98**" in txt and "**Wormhole**" in txt
    ...
    check(bot.name_error("Wormgamer98") is None and bot.name_error("lukas1") is None
    ...
    print("the desk")
    r = desk.claim("Wormgamer98", A)
    p1 = r.password
    check(r.outcome is bot.Outcome.CLAIMED and r.name == "Wormgamer98" and p1
          and pwhash("Wormgamer98") == digest(p1),
          "a name nobody holds: claimed, and the stored digest is Tiger192 of the password sent")
    check(srv.stored_credential("wormgamer98") == srv.tiger192(p1.encode()),
          "...which is the digest the login reply's ticket is encrypted under (any letter case)")
    check(bound_to("Wormgamer98") == A, "...and the name is bound to the Discord user who asked")
    uid = store.db().execute("SELECT user_id FROM accounts WHERE name = 'Wormgamer98'").fetchone()[0]
    check(uid and uid >= 100, f"...with a user id allocated like a console's create ({uid})")

    r = desk.claim("wormgamer98", B)
    check(r.outcome is bot.Outcome.TAKEN and "somebody else" in r.detail
          and pwhash("Wormgamer98") == digest(p1) and bound_to("Wormgamer98") == A,
          "somebody else asking for the same name (any case) is refused; nothing changes")

    r = desk.claim("WORMGAMER98", A)
    p2 = r.password
    check(r.outcome is bot.Outcome.RESET and p2 and p2 != p1 and r.name == "Wormgamer98"
          and pwhash("Wormgamer98") == digest(p2),
    ...
    d = desk.lookup("WORMGAMER98")
    check(d["registered"] and d["name"] == "Wormgamer98" and d["claimed_by"] == A
          and d["handle"] == store.account_handle("wormgamer98"),
